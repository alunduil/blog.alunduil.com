...
does not become the post's subject. Keep the game load-bearing, ~4:1
argument to summary.
...
- **The side stories don't balance.** Six endings, mutually exclusive, with
  faction help that turns you against whoever you didn't help. Follow
  everything and the leads still converge on the Oberon dig site and one
  point of no return, so breadth cashes out as one ending regardless.
  Satisfying in spite of that.
...
The mystery is the recommendation, so the fence is load-bearing.

- **Name the shape, never the answer.** That the leads converge on one site
...
- **Pacing.** 8.5 hours to the main story plus two DLCs. Length reads as
  ending before it wears out rather than dragging; the constraint argument
  depends on that.
- **Images:** body carries the author's capture,
  `Atomfall-2026_03_17-19-45-25.png` (3840×2160). `ogImage` is the game's
  key art at `src/assets/images/atomfall-cover.jpg`, per author decision. A
  second capture from 2026-04-17 exists as a zero-byte file on the share —
  unrecoverable, nothing lost that we can see. No Atomfall clips in the DVR
  folder.
